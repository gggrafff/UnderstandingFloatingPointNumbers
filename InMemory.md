...
Привычный способ записи действительных чисел "0,25" становится не удобным, когда нужно записывать очень большие или очень маленькие числа:
...
1.  знак
2.  порядок (показатель степени) (в виде целого числа в коде со сдвигом)
...
Для приближения ответа к правильному при переполнении, в double можно  записать бесконечное значение. Так же, как и в случае с нулём, для этого используются специальные значение мантиссы и экспоненты.
...
Если вы измеряете размер квартиры, то достаточно `float`. Но если хотите представить координаты GPS с точностью менее метра, то понадобится `double`.
